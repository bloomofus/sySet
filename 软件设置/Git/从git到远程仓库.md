...
- 在git bash的界面中`右击`空白处，弹出菜单，选择`Options->Text->Locale改为zh_CN，Character set改为UTF-8`
...
1. 首先在github创建一个仓库，❌ **不要勾选** “Add a README file”、“Add .gitignore”、“Choose a license”。
...
5. 提交文件保存
   ...
   ```shell
   git commit -m "描述你的更改，例如：添加传感器初始化代码"
   ```
...
### 查看状态 & 差异
   ...
   | `git diff --cached`               | 查看**暂存区 vs 最后一次提交**的差异    |
   | `git log`                         | 查看提交历史（简洁版）                  |
...
### 暂存 & 提交（核心！）
   ...
   | `git add -u`                | 添加所有**已跟踪文件**的修改和删除（不包括新文件）         |
   | `git commit -m "提交信息"`  | 提交暂存区内容到本地仓库                                   |
   | `git commit -am "提交信息"` | **跳过 add**，直接提交所有已跟踪文件的修改（不包括新文件） |

   > ⚠️ `git commit -a` **不会添加新文件（untracked files）**！
...
### 远程仓库操作
   ...
   | `git remote add origin <url>`         | 添加远程仓库（首次）                      |
   | `git remote set-url origin <new-url>` | 修改远程仓库地址（如 HTTPS → SSH）        |
   | `git push -u origin main`             | 首次推送并设置上游分支                    |
   | `git push`                            | 推送当前分支到远程（后续使用）            |
...
### 分支管理
   ...
   | `git switch -c dev`     | （Git 2.23+）同上，更语义化      |
...
### 忽略文件（.gitignore）
   ...
   *.elf
   ...
   > ✅ `.gitignore` 只对**未跟踪文件**生效。已跟踪的文件需先 `git rm -r --cached .` 再提交。
